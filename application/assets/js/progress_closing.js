...
var otherComment = null
if($('#txtOtherComment').length){
    otherComment = CKEDITOR.replace( 'txtOtherComment', {
        wordcount : {
        showCharCount : false,
        showWordCount : true
        },
...
if(($('#mydropzone_4').length) || ($('#mydropzone_5').length) || ($('#mydropzone_8').length) || ($('#mydropzone_9').length)){
    var dropzone_4 = new Dropzone("#mydropzone_4", {
        ...
        url: api + '/upload_file_attach.php?progress=closing&fileposition=4&uid=' + $('#txtUid').val() + '&pid=' + $('#txtPid').val() + '&session_id=' + $('#txtSessionID').val(),
    ...
    var dropzone_5 = new Dropzone("#mydropzone_5", {
        dictDefaultMessage: '<i class="bx bx-upload"></i> อัพโหลดไฟล์ที่นี่',
        url: api + '/upload_file_attach.php?progress=closing&fileposition=5&uid=' + $('#txtUid').val() + '&pid=' + $('#txtPid').val() + '&session_id=' + $('#txtSessionID').val(),
        acceptedFiles: 'application/pdf, .docx, .doc, image/*, .xls, .xlsx',
        maxFilesize: 100,
        init: function(){
            this.on("complete", function(file) {
    ...
    var dropzone_8 = new Dropzone("#mydropzone_8", {
        dictDefaultMessage: '<i class="bx bx-upload"></i> อัพโหลดไฟล์ที่นี่',
        url: api + '/upload_file_attach.php?progress=closing&fileposition=8&uid=' + $('#txtUid').val() + '&pid=' + $('#txtPid').val() + '&session_id=' + $('#txtSessionID').val(),
        acceptedFiles: 'application/pdf, .docx, .doc, image/*, .xls, .xlsx',
        maxFilesize: 100,
        init: function(){
            this.on("complete", function(file) {
    ...
    var dropzone_9 = new Dropzone("#mydropzone_9", {
        dictDefaultMessage: '<i class="bx bx-upload"></i> อัพโหลดไฟล์ที่นี่',
        url: api + '/upload_file_attach.php?progress=closing&fileposition=9&uid=' + $('#txtUid').val() + '&pid=' + $('#txtPid').val() + '&session_id=' + $('#txtSessionID').val(),
        acceptedFiles: 'application/pdf, .docx, .doc, image/*, .xls, .xlsx',
        maxFilesize: 100,
        init: function(){
                this.on("complete", function(file) {
...
var form9 = {
    ...
    addStage3Result(){
        ...
        if($return == '4') // ส่งเจ้าหน้าที่ออกใบรับรอง
        {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var jxr = $.post(api + 'progress?stage=send_staff_tomakedoc', param, function(){}, 'json')
                           .always(function(snap){
                               console.log(snap);
                               if(snap.status == 'Success'){
                                    Swal.fire({
        ...
        if($return == '5') // นำเข้าที่ประชุม
        {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var jxr = $.post(api + 'progress?stage=send_staff_addboard', param, function(){}, 'json')
                           .always(function(snap){
                               console.log(snap);
                               if(snap.status == 'Success'){
                                    Swal.fire({
        ...
        Swal.fire({
            ...
            if (result.value) {
                var jxr = $.post(api + 'progress?stage=confirm_stage3', param, function(){}, 'json')
                           .always(function(snap){
                               console.log(snap);
                               if(snap.status == 'Success'){
                                    Swal.fire({
    ...
    staff_send(){
        ...
        if($val == '1'){
            ...
            if(($('#txtQ2_1').val() == '') || ($('#txtQ2_2').val() == '') || ($('#txtQ2_3').val() == '') || ($('#txtQ2_4').val() == '') || ($('#txtQ2_5').val() == '') || ($('#txtQ2_6').val() == '')){ 
                $msg_arr.push('2.1')
            } 
            if($('#txtQ2_1').val() == ''){
            ...
            if(($('#txtQ3_1').val() == '') || ($('#txtQ3_2').val() == '') || ($('#txtQ3_3').val() == '')){
                $msg_arr.push('2.2')
            }
        ...
        if($val4 == 'na'){ $check++; $msg_arr.push('2') }
        if($val5 == 'na'){ $check++; $msg_arr.push('3')  }
        if($val6 == 'na'){ $check++; $msg_arr.push('4')  }
        if($val7 == 'na'){ $check++; $msg_arr.push('5')  }
        ...
        $summarize = q9.getData()
        $final_report = $('#closing_8').html()
        $manuscript = $('#closing_9').html()

        if(($final_report == '<tr><td colspan="2" class="text-center">ไม่มีไฟล์แนบ</td></tr>') && ($manuscript == '<tr><td colspan="2" class="text-center">ไม่มีไฟล์แนบ</td></tr>') && ($summarize == '')){
            Swal.fire(
                {
                icon: "error",
                title: 'คำเตือน',
                text: 'กรุณาอัพโหลด Final report หรือ Manuscript หรือ สรุปผลการศึกษา อย่างใดอย่างหนึ่ง',
                confirmButtonClass: 'btn btn-danger',
                }
            )
            return ;
        ...
        // return ;
        preload.show()
        var jxr = $.post(api + 'progress_closing?stage=staff_update', param, function(){}, 'json')
                   .always(function(snap){
                       console.log(snap);
                       preload.hide()
                       if(snap.status == 'Success'){
                            Swal.fire({
                                title: 'สำเร็จ',
                                text: "รายงานฉบับนี้ถูกปรับปรุงข้อมูลเรียบร้อยแล้ว",
                                icon: 'success',
                                showCancelButton: false,
                                confirmButtonColor: '#3085d6',
                                cancelButtonColor: '#d33',
    ...
    send(){
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                // return ;
                preload.show()
                var jxr = $.post(api + 'progress_closing?stage=send', param, function(){}, 'json')
                           .always(function(snap){
                               console.log(snap);
                               preload.hide()
                               if(snap.status == 'Success'){
    ...
    autoUpdate(progress){
        ...
        var jxr = $.post(api + 'progress?stage=autosave', param, function(resp){ console.log(resp); })

        
    },
    removeFile(path, role){
        var param = {
            id_rs: $('#txtPid').val(),
            session_id: $('#txtSessionID').val(),
            file_path: path,
        ...
        console.log(param);
        var jxr = $.post(api + 'progress_closing?stage=remove_all_file', param, function(resp){ 
                    getFileProgressSubmissionList($('#txtUid').val(), $('#txtSessionID').val(), 'closing', path)
                  })
    }
}
...
$(function(){
    ...
    $('input[name=radio_4]').click(function(){
        $val = $('input[name=radio_4]:checked').val()
        if($val == '1'){
            $('#hd42').removeClass('dn')
        }else{
            if($('#closing_4').html() == '<tr><td colspan="2" class="text-center">ไม่มีไฟล์แนบ</td></tr>'){
                $('#hd42').addClass('dn')
            }else{
                Swal.fire({
                    title: 'คำเตือน',
                    text: "ไฟล์ที่ถูกอัพโหลดในส่วนนี้ จะถูกลบทั้งหมด",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
    ...
    $('input[name=radio_5]').click(function(){
        $val = $('input[name=radio_5]:checked').val()
        if($val == '1'){
            $('#hd52').removeClass('dn')
        }else{
            if($('#closing_5').html() == '<tr><td colspan="2" class="text-center">ไม่มีไฟล์แนบ</td></tr>'){
                $('#hd52').addClass('dn')
            }else{
                Swal.fire({
                    title: 'คำเตือน',
                    ...
                    if (result.value) {
                        $('#hd52').addClass('dn')
                        form9.removeFile('5', 'pi')
                    }else{
                        $('#radio_5_2').prop('checked', 'checked')
                    }
                })
            }
...
function getFileProgressSubmissionList(uid, session_id, progress, filepath){
    ...
    var jxr = $.post(api + 'progress?stage=get_submission_file', param, function(){}, 'json')
                           .always(function(snap){
                               ...
                               if(snap.status == 'Success'){
                                    ...
                                    snap.data.forEach(i => {
                                        ...
                                        $stf_btn = '';
                                        if($('#txtRole').val() == 'staff'){
                                            $stf_btn = '<button type="button" class="btn btn-outline-success btn-icon btn-sm" ' + $ald + '  style="padding-top: 3px; padding-bottom: 10px; margin-right: 4px;" onclick="openRenameModal(\'' + i.rpfs_id + '\', \'' + i.rpfs_name + '\')"><i class="bx bx-pencil"></i></button>';
                                            $stf_btn += '<button type="button" class="btn btn-outline-danger btn-icon btn-sm" ' + $ald + '  style="padding-top: 3px; padding-bottom: 10px;" onclick="deleteFile(\'' + i.rpfs_id + '\', \'' + progress + '\', \'' + filepath + '\')"><i class="bx bx-trash"></i></button>';
                                        }else if($('#txtRole').val() == 'pm'){
                                            if(i.rpfs_allow_delete == '0'){
                                                $stf_btn += '<button type="button" disabled class="btn btn-outline-danger btn-icon btn-sm" ' + $ald + '  style="padding-top: 3px; padding-bottom: 10px;" onclick="deleteFile(\'' + i.rpfs_id + '\', \'' + progress + '\', \'' + filepath + '\')"><i class="bx bx-trash"></i></button>';
                                            }else{
                                                $stf_btn += '<button type="button" class="btn btn-outline-danger btn-icon btn-sm" ' + $ald + '  style="padding-top: 3px; padding-bottom: 10px;" onclick="deleteFile(\'' + i.rpfs_id + '\', \'' + progress + '\', \'' + filepath + '\')"><i class="bx bx-trash"></i></button>';
                                            }
                                        }
                                        $('#' + progress + '_' + filepath).append(
                                            '<tr>' + 
                                                '<td>' + $c + '. ' + i.rpfs_name +
                                                '<div>' + 
                                                    '<button type="button" class="btn btn-outline-success btn-icon btn-sm" style="padding-top: 3px; padding-bottom: 10px; margin-right: 4px;" onclick="openFile(\'' + i.rpfs_url + '\')"><i class="bx bx-download"></i></button>'  +    
                                                    $stf_btn +
                                                '</div></td>' + 
                                            '</tr>'
                                        )
...
function openRenameModal(id, fname){
    $('#txtRenameId').val(id)
    $('#txtRenamePrev').val(fname)
    $('#txtRenameNew').val('')
...
function confRename(){
    if($('#txtRenameNew').val() == ''){
        $('#txtRenameNew').addClass('is-invalid');
        return ;
    }
    preload.show()
    var param = {
        uid: $('#txtUid').val(),
        role: $('#txtRole').val(),
        file_id: $('#txtRenameId').val(),
        new_name: $('#txtRenameNew').val(),
        id_rs: $('#txtResearchId').val(),
        session_id: $('#txtSessionId').val()
    }
    // console.log(param);
    // return ;
    var jxr = $.post(api + 'progress?stage=rename_file', param, function(){}, 'json')
                           .always(function(snap){
                               console.log(snap);
                                preload.hide()
...
function deleteFile(file_id, progress, file_path){
    Swal.fire({
        ...
        if (result.value) {
            ...
            var jxr = $.post(api + 'progress?stage=delete_submission_file', param, function(){}, 'json')
                                   .always(function(snap){
                                       if(snap.status == 'Success'){
...
var f9ass = {
    ...
    saveAssesmentDraft(){
        ...
        var jxr = $.post(api + 'progress?stage=assesment_submission', param, function(snap){ console.log(snap); }, 'json')
    ...
    saveAssesment(){
        ...
        // if($('#txtRole').val() == 'ec'){
            var jxr = $.post(api + 'progress?stage=assesment_submission', param, function(){}, 'json')
                       .always(function(snap){
                           console.log(snap);
                           if(snap.status == 'Success'){
    ...
    saveOtherComment(){
        ...
        var param = {
            uid: $('#txtUid').val(),
            sid: $('#txtSessionID').val(),
            comment: otherComment.getData()
        }
        var jxr = $.post(api + 'progress?stage=save_other_comment', param, function(){}, 'json')
                    .always(function(snap){
                        console.log(snap);
                        preload.hide()
                        if(snap.status == 'Success'){
                            $('#modalOtherComment').modal('hide')
                            f9ass.listCommentStaff()
                        }else{
                            Swal.fire({
                                icon: "error",
    ...
    saveComment(){
        ...
        var jxr = $.post(api + 'progress?stage=reviewer_comment_submission', param, function(){}, 'json')
                       .always(function(snap){
                           console.log(snap);
                           if(snap.status == 'Success'){
    ...
    listCommentStaff(){
        ...
        var jxr = $.post(api + 'progress?stage=reviewer_comment_list_staff', param, function(){}, 'json')
                       .always(function(snap){
                           console.log(snap);
                           ...
                           if(snap.status == 'Success'){
                            $('#commentList').html('')
                                ...
                                snap.data.forEach(i=>{
                                    ...
                                    $title = i.recc_title
                                    if(i.recc_title == null){
                                        $title = 'อื่น ๆ'
                                    }
                                    
                                    $('#commentList').append('<tr><td style="vertical-align: top;">' + $c + '</td><td><div><span class="badge badge-secondary round">Reviewer : ' + i.recc_id_reviewer + '</span></div><div class="pl-0 pr-0 pt-1"><div>ประเด็น : ' + $title + '<br>-----------------------</div><div class="text-dark">' + i.recc_comment + '</div></td><td class="text-right" style="vertical-align: top;">' + 
                                        '<button class="btn btn-secondary btn-icon btn-sm" style="padding-top: 8px; padding-bottom: 10px; margin-right: 4px;" onclick="f9ass.updateComment(' + i.recc_id + ')"><i class="bx bx-pencil"></i></button>'  +
                                        '<button class="btn btn-warning btn-icon btn-sm" ' + $btnMoveup + ' style="padding-top: 8px; padding-bottom: 10px; margin-right: 4px;" onclick="f9ass.updateSeq(' + i.recc_id + ', 0, ' + i.recc_seq + ', ' + i.recc_up_to + ', ' + i.recc_down_to + ')"><i class="bx bx-up-arrow-alt"></i></button>'  +
                                        '<button class="btn btn-warning btn-icon btn-sm" ' + $btnMovedown + ' style="padding-top: 8px; padding-bottom: 10px; margin-right: 4px;" onclick="f9ass.updateSeq(' + i.recc_id + ', 1, ' + i.recc_seq + ', ' + i.recc_up_to + ', ' + i.recc_down_to + ')"><i class="bx bx-down-arrow-alt"></i></button>'  +
                                        '<button class="btn btn-danger btn-icon btn-sm" style="padding-top: 8px; padding-bottom: 10px;" onclick="f9ass.deleteComment(' + i.recc_id + ')"><i class="bx bx-trash"></i></button>'  +
                                    '</td></tr>')
                                    $c++;
                                })
                           }
    ...
    deleteComment(id_recc){
        Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var jxr = $.post(api + 'progress?stage=reviewer_comment_delete', param, function(){}, 'json')
                                       .always(function(snap){
                                           console.log(snap);
                                            f9ass.listCommentStaff()
                                       })
    ...
    updateSeq(id_recc, upordown, curr_seq, upto, downto){
        ...
        var jxr = $.post(api + 'progress?stage=reviewer_comment_changeseq', param, function(){}, 'json')
                       .always(function(snap){
                           console.log(snap);
                            f9ass.listCommentStaff()
                       })
    ...
    updateComment(comment_id){
        ...
        var jxr = $.post(api + 'progress?stage=reviewer_comment_forupdate', param, function(){}, 'json')
                       .always(function(snap){
                           console.log(snap);
                            $('#txtCommentId').val('')
