      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>โครงการวิจัยรอการแก้ไขในขั้นตอนการตรวจสอบความถูกต้องเอกสาร</h1>
          </div>
                          ...
                          <tbody>
                              <?php
                                ...
                                WHERE
                                  ...
                                  AND g.user_id = '$uid'
                                  ORDER BY a.date_submit LIMIT $start_row, $row_per_page";
                                    ...
                                    while($row = mysqli_fetch_array($result)){
                                        ...
                                        <tr>
                                            ...
                                            <td style="vertical-align: top; padding: 10px;" class="pb-3">
                                                <div><a href="Javascript:project.set2update1('<?php echo $row['id_rs']; ?>')" class="text-dark"><?php echo $title;?></a></div>
                                                ...
                                                <div class="fs08">สถานะโครงการ : <span class="text-danger"><?php echo $row['status_name'];?></span></div>
                                                <div class="pt-1">
                                                    <button class="btn btn-primary btn-icon- btn-sm" style="box-shadow: none; width-: 28px;" onclick="project.set2EcMessage('<?php echo $row['id_rs']; ?>')"  data-toggle="modal" data-target="#messageModal"><i class="fas fa-comment"></i> 1. ข้อความจากสำนักงาน</button>
                                                    <?php
                                                    if($row['id_status_research'] == '2'){
                                                      ?>
                                                      <button class="btn btn-primary btn-icon btn-sm" style="box-shadow: none; width+: 28px;" onclick="project.set2update1('<?php echo $row['id_rs']; ?>')"><i class="fas fa-wrench"></i> 2. แก้ไข</button>
                                                      <?php
                                                    }else{
                                                      ?>
                                                      <button class="btn btn-primary btn-icon btn-sm" style="box-shadow: none; width+: 28px;" onclick="initial.manageProject('<?php echo $row['id_rs']; ?>', './research_register_update?uid=<?php echo $uid;?>&role=<?php echo $role?>&id_rs=<?php echo $row['id_rs'];?>')"><i class="fas fa-wrench"></i> 2. อ่านความเห็นกรรมการเพื่อแก้ไข</button>
                                                      <?php
                                                    }
                                                    ?>
                                                    <button class="btn btn-danger btn-icon btn-sm" style="box-shadow: none; width-: 28px;" onclick="project.set2deleteProject('<?php echo $row['id_rs']; ?>')"><i class="fas fa-times"></i> ถอนโครงการ</button>
                                                </div>
                                            </td>
...
<div class="modal fade" id="messageModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <h5 class="modal-title pb-3 text-white" id="exampleModalLabel">ข้อความล่าสุดจากสำนักงาน</h5>
        <button type="button" class="close text-white btnCloseModal" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body" id="ecMessageArea">
        <div class="text-center pt-5 pb-4"><i class="fas fa-sync fa-spin" style="font-size: 5em;"></i></div>
      </div>
    </div>
  </div>
</div>
  ...
  <script>
      ...
      $(function(){
          $('#txtFilteryear').change(function(){
              $selected = $('#txtFilteryear').val()
              if($selected != ''){
                  $select_year = (parseInt($selected) + 1999);
                  window.location = 'rs-init-list?uid=' + current_user + '&role=' + current_role + '&page=<?php echo $cpage;?>&year=' + $select_year
              }
          })
      })
  </script>
