            ...
            <div class="content-header row">
                <div class="content-header-left col-12 mb-2 mt-1">
                    <div class="breadcrumbs-top">
                        <h5 class="content-header-title float-left pr-1 mb-0 text-dark">โครงการที่อยู่ระหว่างการดำเนินการ (สำหรับเจ้าหน้าที่)</h5>
                ...
                <section id="card-navigation">
                    <div class="row">
                        <div class="col-12 col-sm-4">
                            <button class="btn text-left btn-<?php 
                            $strSQL = "SELECT COUNT(*) cn FROM rec_progress WHERE rp_progress_status = '1' AND rp_sending_status = '1' AND rp_delete_status = '0'";
                            ...
                            ?> btn-lg btn-block" onclick="window.location = 'progress_btn1'"><i class="bx bx-check-double"></i> รอตรวจสอบความถูกต้องของเอกสาร <?php if($n > 0){ echo "(".$n.")"; } ?></button>

                            
                            <br>
                            <br>

                            <button class="btn text-left btn-success btn-lg btn-block" onclick="window.location = 'progress_waiting'"><i class="bx bx-time"></i> รอออกใบรับรอง/รับทราบ <?php if($n > 0){ echo "(".$n.")"; } ?></button>
                            <button class="btn text-left btn-success btn-lg btn-block" onclick="window.location = 'progress_waiting'"><i class="bx bx-time"></i> รอขอข้อมูลเพิ่มเติมจากนักวิจัย <?php if($n > 0){ echo "(".$n.")"; } ?></button>

                            <button class="btn text-left btn-success btn-lg btn-block" onclick="window.location = 'progress_waiting'"><i class="bx bx-time"></i> รอทาบทามกรรมการพิจารณา <?php if($n > 0){ echo "(".$n.")"; } ?></button>
                            <button class="btn text-left btn-success btn-lg btn-block" onclick="window.location = 'progress_waiting'"><i class="bx bx-time"></i> รอสรุปผลประเมินเพื่อส่งเลขา <?php if($n > 0){ echo "(".$n.")"; } ?></button>
                            <button class="btn text-left btn-success btn-lg btn-block" onclick="window.location = 'progress_waiting'"><i class="bx bx-time"></i> รอนักวิจัยตอบข้อเสนอแนะ <?php if($n > 0){ echo "(".$n.")"; } ?></button>
                            
                            <button class="btn text-left btn-success btn-lg btn-block" onclick="window.location = 'progress_waiting'"><i class="bx bx-time"></i> รอนำเข้าที่ประชุม <?php if($n > 0){ echo "(".$n.")"; } ?></button>
                            <button class="btn text-left btn-success btn-lg btn-block" onclick="window.location = 'progress_waiting'"><i class="bx bx-time"></i> รอดำเนินการอื่น ๆ <?php if($n > 0){ echo "(".$n.")"; } ?></button>


                            <button class="btn text-left btn-<?php 
                            $strSQL = "SELECT COUNT(*) cn FROM rec_wait_staff_progress WHERE rwp_status = '1' AND rwp_delete = '0' GROUP BY rwp_session_id";
                            $res = $db->fetch($strSQL,false);
                            $n = 0;
                            if(($res) && ($res['cn'] > 0)){
                                $n = $res['cn'];
                            ...
                            ?> btn-lg btn-block"><i class="bx bxs-group"></i> รอสรุปผลจากที่ประชุม <?php if($n > 0){ echo "(".$n.")"; } ?></button>

                            <button class="btn text-left btn-success btn-lg btn-block"><i class="bx bx-table"></i> รายงานการดำเนินการวิจัยทั้งหมด <?php if($n > 0){ echo "(".$n.")"; } ?></button>
