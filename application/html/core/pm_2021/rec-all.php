    ...
    <div class="app-content content pt-2">
        ...
        <div class="content-wrapper pl-0 pr-0 pl-sm-3 pr-sm-3">
            ...
            <div class="content-header row">
                <div class="content-header-left col-12 mb-2 mt-1 pl-sm-1 pr-sm-1 pl-3 pr-3 pt-0 pt-sm-0">
                    <h3 class="text-dark  mb-0">โครงการวิจัยที่อยู่ระหว่างดำเนินการของท่าน</h3>
                    <div class="breadcrumbs-top">
                        <div class="breadcrumb-wrapper d-none d-sm-block">
                            <ol class="breadcrumb p-0 mb-0 pl-1">
                                <li class="breadcrumb-item"><a href="./"><i class="bx bx-home-alt"></i> หน้าแรก</a></li>
                                <li class="breadcrumb-item active">โครงการวิจัยที่อยู่ระหว่างดำเนินการวิจัย</li>
                            </ol>
                        </div>
                    </div>
                </div>
            ...
            <div class="content-body" >
                                ...
                                <table class="table table-stiped">
                                    <thead>
                                        <tr class="bg-secondary">
                                            ...
                                            <th style="font-size: 1em !important; width: 170px;" class="text-white">รายงานที่อยู่ระหว่างการยื่น</th>
                                        </tr>
                                    </thead>
                                            ...
                                            foreach ($recCheck['data'] as $row) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="vertical-align: top;">
                                                        ...
                                                        if($row['title_th'] != '-'){
                                                            ?>
                                                            <a href="" class="text-dark"><?php echo $row['title_th']. "<br><small class='text-muted'>(".$row['title_en'].")</small>"; ?></a>
                                                            <?php
                                                        }else{
                                                            echo $row['title_en'];
                                                        }
                ...
                <?php 
                    if(($recCheck) && ($recCheck['count'] > 0)){
                        foreach ($recCheck['data'] as $row) {
                            ?>
                            <div class="card bg-white" style="margin-bottom: 3px;">
                                <div class="card-body">
                                    <div><span class="badge badge-sm badge-danger">REC.<?php echo $row['code_apdu']; ?></span></div>
                                    ...
                                    if($row['title_th'] != '-'){
                                        ...
                                        ?>
                                        <a href="research-register?id_rs=<?php echo $row['id_rs']; ?>" class="text-dark"><?php echo $row['title_en']; ?></a>
                                        <?php
                                    }
                                    ?>
                                    </div>
                                    <div class="pt-1">
                                        <button class="btn btn-success" onclick="createReport('<?php echo $row['id_rs']; ?>')"><i class="bx bx-plus"></i> สร้างรายงาน</button>
                                        <button class="btn btn-secondary" onclick="window.location = 'rec-report-list?id_rs=<?php echo $row['id_rs']; ?>'"><i class="bx bx-search"></i> ดูข้อมูล</button>
                                    </div>
                                </div>
                            </div>
                            <?php
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="pt-3 pb-3">
                                    ไม่พบโครงการที่อยู่ระหว่างการดำเนินการวิจัย
                            </div>
                        </div>
                        <?php
                    }
            ...
            var jxr = $.post(api + 'api?stage=create_progress_form', param, function(){}, 'json')
                       .always(function(snap){
                           if(snap.status == 'Success'){
                               console.log(snap);
                            //    return ;
                                window.location = 'progressform_' + $('#txtReportype').val() + '_create?id_rs=' + recent_selected_project + '&session_id=' + snap.session_id
