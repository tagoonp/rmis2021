        ...
        <div class="content-wrapper">
            ...
            <div class="content-header row">
                <div class="content-header-left col-12 mb-2 mt-1 pl-sm-1 pr-sm-1 pl-3 pr-3 pt-2 pt-sm-0">
                    <h3 class="text-white">รายงานที่รอการแก้ไข/ตอบข้อเสนอแนะ</h3>
                            ...
                            <table class="table table-stiped">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="font-size: 1em !important; width: 370px;" class="text-white">รายงานที่รอแก้ไข/ตอบข้อเสนอแนะ</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php 
                                    $strSQL = "SELECT * FROM research 
                                               WHERE id_pm = '$my_id_pm' 
                                               AND code_apdu != '' 
                                               AND delete_flag = 'N' 
                                               AND id_status_research = '18' 
                                               AND id_rs IN 
                                                (
                                                    SELECT rp_id_rs 
                                                    FROM rec_progress
                                                     WHERE rp_uid = '$uid' 
                                                     AND rp_progress_status IN ('2', '20')
                                    ...
                                    if(($recCheck) && ($recCheck['count'] > 0)){
                                        foreach ($recCheck['data'] as $row) {
                                            ...
                                            <tr>
                                                ...
                                                <td style="vertical-align: top;">
                                                    <?php 
                                                    $strSQL = "SELECT * FROM rec_progress
                                                                WHERE rp_uid = '$uid' 
                                                                AND rp_progress_status IN ('2', '20')
                                                                AND rp_sending_status = '1' 
                                                                AND rp_delete_status = '0'
                                                                AND rp_id_rs = '".$row['id_rs']."'
                                                                ";
                                                    $resPr = $db->fetch($strSQL, true, true);
                                                    if(($resPr) && ($resPr['status'])){
                                                        foreach ($resPr['data'] as $row2) {
                                                            ?>
                                                            <button class="btn btn-danger btn-block" style="margin-bottom: 4px;" onclick="window.location = 'progressform_closing_create?id_rs=<?php echo $row['id_rs'];?>&session_id=<?php echo $row2['rp_session']; ?>'"><?php echo $row2['rp_progress_id']."-".$row2['rp_session']; ?></button>
                                                            <?php
                                                        }
                                                    }
                                                    ?>
