...
$strSQL = "SELECT * FROM research a INNER JOIN type_research b ON a.id_type = b.id_type WHERE a.id_rs = '$id_rs'";
...
<body class="vertical-layout vertical-menu-modern dark-layout 2-columns  navbar-sticky footer-static  " data-open="click" data-menu="vertical-menu-modern" data-col="2-columns" data-layout="dark-layout">
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header row">
                <div class="content-header-left col-12 mb-2 mt-1">
                    <h3 class="text-dark">รายงานการดำเนินการวิจัย</h3>
            ...
            <div class="content-body">
                <div class="row">
                    <div class="col-12 col-sm-9">
                        <?php 
                        if($stage == 1){ require('./comp/rs_info.php'); }
                        if($stage == 2){ require('./comp/rs_doc.php'); }
                        if($stage == 3){ require('./comp/rs_reviewer.php'); }
                        if($stage == 4){ require('./comp/rs_comment.php'); }
                        if($stage == 5){ require('./comp/rs_progress_report.php'); }
                        if($stage == 7){ require('./comp/rs_note.php'); }
                        if($stage == 8){ require('./comp/rs_init_activity.php'); }
                        if($stage == 9){ require('./comp/rs_cont_activity.php'); }
                        ?>
                    </div>
                    <div class="col-12 col-sm-3">
                        <h5 class="text-dark">Initial review information</h5>
                        <div class="list-group">
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 1){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>'">1. ข้อมูลโครงการวิจัย</button>
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 2){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>&stage=2'">2. เอกสารโครงการ</button>
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 3){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>&stage=3'">3. ผู้เชียวชาญอิสระ</button>
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 4){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>&stage=4'">4. บันทึกข้อเสนอแนะ</button>
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 5){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>&stage=5'">5. รายงานการดำเนินการวิจัย</button>
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 6){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>&stage=6'">6. เอกสารรับรอง/รับทราบ</button>
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 7){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>&stage=7'">7. Note</button>
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 8){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>&stage=8'">8. Activity logs (Initial)</button>
                            <button type="button" class="list-group-item list-group-item-action <?php if($stage == 9){ echo "active"; } ?>" onclick="window.location='research-info?id_rs=<?php echo $id_rs; ?>&stage=9'">9. Activity logs (Continuing)</button>
                        </div>

                        <h5 class="text-dark mt-3">คำสั่งดำเนินการ</h5>
                        <div class="list-group">
                            <button type="button" class="list-group-item list-group-item-action"><i class="bx bx-refresh"></i> เปลี่ยนเลขา ฯ รับผิดชอบ</button>
                        </div>
    ...
    <script src="../../../assets/js/initial_review.js?v=<?php echo filemtime('../../../assets/js/initial_review.js'); ?>"></script>
