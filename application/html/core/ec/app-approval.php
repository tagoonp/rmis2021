                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-striped- mb-0">
                                    <thead>
                                        <tr>
                                            <th style="width: 200px;">รหัสใบรับรอง</th>
                                            <th>Ref. code</th>
                                            <th>วันที่สร้าง</th>
                                            <th>วันที่รับรอง</th>
                                            <th>วันหมดอายุ</th>
                                    ...
                                    <tbody>
                                        <?php 
                                        $strSQL = "SELECT * FROM rec_approval_info WHERE rai_id_rs = '$id_rs' AND rai_delete = '0'";
                                        $resDoc = $db->fetch($strSQL, true, false);
                                        if(($resDoc) && ($resDoc['status'])){
                                            foreach ($resDoc['data'] as $row) {
                                                ?>
                                                <tr>
                                                    <td><?php echo $row['rai_progress']."-".$row['rai_session_id']; ?></td>
                                                    <td><?php echo $row['rai_doc_session_id']; ?></td>
                                                    <td><?php echo date('d/m/Y', strtotime($row['rai_staff_udate'])); ?></td>
                                                    <td><?php echo date('d/m/Y', strtotime($row['rai_sign_date'])); ?></td>
                                                    <td><?php if($row['rai_expired_date'] != null){ echo date('d/m/Y', strtotime($row['rai_expired_date'])); }else{ echo "-"; }  ?></td>
                                                    <td>
                                                    <?php 
                                                        if($row['rai_review_status'] == 'draft'){
                                                            ?>
                                                            <span class="badge badge-light-warning round">รอตรวจสอบ</span>
                                                            <?php
                                                        }else if($row['rai_review_status'] == 'in-progress'){
                                                            ?>
                                                            <span class="badge badge-light-warning round">อยู่ระหว่างดำเนินการ</span>
                                                            <?php
                                                        }else if($row['rai_review_status'] == 'in-progress'){
                                                            ?>
                                                            <span class="badge badge-light-success round">ลงนามแล้ว</span>
                                                            <?php
                                                        }
                                                        ?>
                                                    </td>
                                                    <td>
                                                        <?php 
                                                        if($row['rai_sign_status'] == '1'){
                                                            ?>
                                                            <button class="btn btn-icon btn-success" style="padding-bottom: 10px; margin-top: 3px;" onclick="window.location='view_doc_<?php echo $row['rai_progress'] ;?>?id_rs=<?php echo $id_rs; ?>&session_id=<?php echo $session_id; ?>'"><i class="bx bx-search"></i></button>
                                                            <?php
                                                        }else{
                                                            ?>
                                                            <button class="btn btn-icon btn-success" style="padding-bottom: 10px; margin-top: 3px;" onclick="window.location='create_doc_<?php echo $row['rai_progress'] ;?>?id_rs=<?php echo $id_rs; ?>&session_id=<?php echo $session_id; ?>'"><i class="bx bx-search"></i></button>
                                                            <?php
                                                        }
                                                        ?>
                                            ...
                                            ?>
                                            <tr>
                                                <td colspan="5" class="text-center">ยังไม่มีใบรับรอง</td>
                                            </tr>
                                            <?php
                                        }
                                        ?>
